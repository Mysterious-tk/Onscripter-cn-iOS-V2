...
void ScriptParser::reset()
{
	...
	if (!useencrypt) {
        setStr( &save_menu_name, "保存" );
        setStr( &load_menu_name, "读取" );
        setStr( &save_item_name, "进度" );
    }
    else {
        setStr( &save_menu_name, "＊娈" );
        setStr( &load_menu_name, "两∫" );
        setStr( &save_item_name, "冉" );
    }
    num_save_file = 9;
