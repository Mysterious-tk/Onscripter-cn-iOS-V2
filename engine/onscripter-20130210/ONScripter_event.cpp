...
static SDL_TimerID timer_save = NULL;
...
extern "C" Uint32 SDLCALL ONScripter::saveCallback( Uint32 interval, void *param )
{
...
bool ONScripter::keyPressEvent( SDL_KeyboardEvent *event )
{
         ...
         // }
          ...
          //}
           ...
           if( system_menu_mode = SYSTEM_WINDOWERASE)
            ...
            if ( WAIT_TEXT_MODE){
		if(autosave){
              	timer_save = SDL_AddTimer( 180000, saveCallback, (void*)this  );
			autosave = false;
		}
		else{
			SDL_RemoveTimer(timer_save);
			autosave = true;
		}
          }
        }
	else if ( !useescspc_flag && event->keysym.sym == SDLK_u){
					...
					const char *buf = ".bmp";
					int shour=tm->tm_hour;
					int smin=tm->tm_min;
					int ssec=tm->tm_sec;
					int sticks=SDL_GetTicks();
					char sname[50];
					sprintf( sname, "%d%d%d%d%s", shour,smin,ssec,sticks, buf );
						char filename[256];
						sprintf( filename, "%s%s", archive_path, sname );
	...
	else if ( !useescspc_flag && event->keysym.sym == SDLK_y){
				...
				if (wide_screen_mode){
					...
					swidth = (modes[0]->h / 9) * 16;
				    else
					swidth = (modes[0]->w / 16) * 16;
				}
				else{
				    if (modes[0]->w * 3 > modes[0]->h * 4)
					{
					swidth = (modes[0]->h / 3) * 4;
					if(modes[0]->h == 640)
					swidth +=1;
					}
				    else
					swidth = (modes[0]->w / 4) * 4;
				}
				if(scale_screen_mode)
				{
					screenshot_w = modes[0]->w;
					screenshot_h = modes[0]->h;
				}
				else
				{
					screenshot_w = swidth;
					screenshot_h = swidth*3/4;
				}
