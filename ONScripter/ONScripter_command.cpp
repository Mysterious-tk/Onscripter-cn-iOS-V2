...
int ONScripter::vibratorCommand()
{
#ifdef ANDROID
	int time = script_h.readInt();
	vibratorAndroid(time);
	//printf("%d",time);
#endif
	return RET_CONTINUE;
}
...
/*
	if(font_x>0)
	sentence_font.font_size_xy[0] = font_x;
	if(font_y>0)
    sentence_font.font_size_xy[1] = font_y;
	if(font_px>0)
	sentence_font.pitch_xy[0] = font_px;
	if(font_py>0)
	sentence_font.pitch_xy[1] = font_py;
*/
    ...
    if ( buf[0] == '#' ){
		...
		if( screen_ratio1 == 385 || screen_ratio1 == 479)
		ai->orig_pos.w += ai->orig_pos.w / 4;
		else if(screen_ratio1 == 384 || screen_ratio1 == 481 || screen_ratio1 == 865 || screen_ratio1 == 1081 || screen_ratio1 == 1537 || screen_ratio1 == 1921)
		ai->orig_pos.w += ai->orig_pos.w / 3;
		else if (screen_ratio1 == 256 || screen_ratio1 == 320 || screen_ratio1 == 1024 || screen_ratio1 == 1280)
		ai->orig_pos.w += ai->orig_pos.w / 7.5;
		else if(screen_ratio1 == 1025 || screen_ratio1 == 1281)
		ai->orig_pos.w += ai->orig_pos.w / 3.5;
		//printf("%d\n",ai->orig_pos.w);
...
extern "C" Uint32 SDLCALL ONScripter::loopbgmCallback( Uint32 interval, void *param )
{
	...
	{
		loopbgm = true;
		ONScripter* obj = ( ONScripter* )param;
...
int ONScripter::loopbgm2Command()
{
	if(!loopbgm)
	{
    const char *buf = script_h.readStr();
	...
	if(loopbgm){
		...
		playSound(loop_bgm_name[0], 
					  ...
					  true, MIX_LOOPBGM_CHANNEL1);
		music_play_loop_flag = false;
	}
	if(!loopbgm)
	playSound(loop_bgm_name[1], 
                  SOUND_PRELOAD |SOUND_MUSIC |SOUND_CHUNK,
