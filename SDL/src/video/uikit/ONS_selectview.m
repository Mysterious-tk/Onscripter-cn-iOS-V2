...
-(void)loadGame:(NSString *)path{
    ...
    forward_argv[3] = (char*)malloc(sizeof("--render-font-outline"));
    strcpy(forward_argv[3], "--bender-font-outline");//change for outline support
    if (btnWide) {
        forward_argc++;
        forward_argv[forward_argc-1] = (char*)malloc(sizeof(char)*7);
        strcpy(forward_argv[forward_argc-1], "--wide");
    }
    if (btnIsFull) {
        forward_argc++;
